...
def build_demo_database(db_path: Path, data_dir: Path, reference_date: date) -> None:
    """Create a fresh SQLite demo database from the source CSVs and enrich it with synthetic records."""
...
def seed_invoices(
    ...
    """Create invoice headers + line items, and ensure each invoice links back to an appointment and procedure."""
    line_item_id = 1
    doctor_ids = sorted(doctor_facilities)
