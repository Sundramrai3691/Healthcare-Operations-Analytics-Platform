...
This is no longer just a SQL exercise or a CSV viewer. It is positioned as a simple hospital analytics demo with:

- live API-backed filtering
- facility-aware scheduling and billing views
...
- A small smoke test validates the core API routes and demo-admin write flow before pushing changes
...
- GitHub Pages serves the `docs/` portfolio site
...
**Healthcare operations analytics: schema design, reporting logic, and a clean reporting surface.**

That story is reflected in the interface:

- dashboard overview metrics
- billing trend tracking
...
- Fast to load: yes, because the app uses a small SQLite database and a simple frontend
- Public hosting setup included: GitHub Pages workflow for `docs/`
- Interactive hosting setup: not configured yet, but the Python app is ready for a second hosting option later

As a static fallback, the `docs/` folder can still be hosted on GitHub Pages.
...
The interactive Python dashboard is ready to host separately after the GitHub Pages portfolio link is in place.
