...
SELECT
    f.FacilityName,
    substr(i.IssuedAt, 1, 7) AS RevenueMonth, -- Extract YYYY-MM from the invoice timestamp
...
SELECT
    ...
    -- Percent of invoices marked as Settled for each payer
...
FROM Facilities f
-- LEFT JOINs keep facilities visible even when there are no matching events
LEFT JOIN Appointments a ON a.FacilityID = f.FacilityID
LEFT JOIN LabResults lr ON lr.FacilityID = f.FacilityID
-- Prescriptions link through appointments (so a facility with no visits has no prescriptions)
...
SELECT
    ...
    -- Patient out-of-pocket portion captured on the invoice header
