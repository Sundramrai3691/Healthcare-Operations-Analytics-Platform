...
def create_demo_encounter(payload: dict) -> dict:
    """Insert a demo patient encounter (and related records) used by the UI's admin panel."""
    required_fields = [
        "firstName",
        "lastName",
        "facilityId",
...
def get_overview(params: dict[str, list[str]]) -> dict:
    """Return the dashboard KPI summary for the current filter slice (facility/specialty)."""
...
def get_patient_journey(params: dict[str, list[str]]) -> dict:
    """Return a patient profile plus a unified timeline of appointments, procedures, labs, medications, and invoices."""
    ...
    # Build a single timeline by UNIONing the main event types tied to the patient.
    timeline = query_all(
        """
        SELECT
            eventDate,
